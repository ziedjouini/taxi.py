...
if genre == 'USA':
    ...
    df1 
    viz_correlation=sns.heatmap(df1.corr(), center=0,cmap = sns.color_palette("vlag", as_cmap=True))
    st.pyplot(viz_correlation.figure)
    st.line_chart(df1['hp'])
if genre == 'Europe':
    df2 = df_voiture.loc[df_voiture["continent"] ==" Europe."] 
    df2
    viz_correlation=sns.heatmap(df2.corr(), center=0,cmap = sns.color_palette("vlag", as_cmap=True))
    st.pyplot(viz_correlation.figure)
    st.line_chart(df2['hp'])
else:
    df3 = df_voiture.loc[df_voiture["continent"] ==" Japan."]
    df3
    viz_correlation=sns.heatmap(df3.corr(), center=0,cmap = sns.color_palette("vlag", as_cmap=True))
    st.pyplot(viz_correlation.figure)
